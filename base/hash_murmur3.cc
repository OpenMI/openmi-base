...
void MMHash128::hash128(void const* key, const int size, void* hash, uint32_t seed) {
  uint64_t *h1 = reinterpret_cast<uint64_t *>(hash);
  uint64_t *h2 = h1 + 1;
  return hash128(key, size, h1, h2, seed);
}

void MMHash32::hash32(const void* key, const int len, void* hash, const uint32_t seed) {
  // ------------- initialization -------------
  uint32_t h = seed, k;
...
uint32_t MMHash32::hash32(const void* key, const int len, const uint32_t seed) {
  uint32_t hash;
